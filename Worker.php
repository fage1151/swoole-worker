...
class Worker
{
    ...
    protected static $_availableEventLoops = array(
        'Swoole'   =>'\Workerman\Events\Swoole_event',
    );
    ...
    protected static function parseCommand()
    {
        ...
        if ($master_is_alive) {
            if ($command === 'start' && getmypid() != $master_pid) {
    ...
    protected static function forkOneWorker($worker)
    {
        ...
        $process = new swoole_process(function(swoole_process $process) use($worker,$id){
            ...
            Timer::delAll();
            $process->name('WorkerMan: worker process  ' . $worker->name . ' ' . $worker->getSocketName());
    ...
    protected static function writeStatisticsToStatusFile()
    {
        ...
        /** @var Worker $worker */
        $worker           = current(self::$_workers);
        $worker_status_str = getmypid() . "\t" . str_pad(round(memory_get_usage(true) / (1024 * 1024), 2) . "M",
    ...
    public static function checkErrors()
    {
        if (self::STATUS_SHUTDOWN != self::$_status) {
            $error_msg = 'Worker['. getmypid() .'] process terminated with ';
